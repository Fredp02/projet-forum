...
class RegisterController extends MainController
{
    ...
    public function register($action)
    {
        /**
         * ! -> 1
         * * Affichage de la vue pour s'inscrire : $this->registerView();
         * 
         * ! -> 2 
         * * Envoi du mail avec token jwt : $this->createAccount(...)
         * ! -> 2bis 
         * * Renvoie du mail avec token jwt : $this->returnToken($userId);
         * 
         * ! -> 3
         * * Si token jwt valide, activation :  $this->activationAccount(...)                
         */

        /**
         * ! 1 -> register/viewRegister
         * ! 2 -> register/sendMail
         * ! 2bis -> register/resendMail
         * ! 3 -> register/accountActivation
         */
        ...
        if (!Securite::isConnected()) {
            ...
            if (isset($url[2]) && !empty($url[2])) {
                if ($url[2] === "returnToken" && isset($url[3]) && !empty($url[3])) { // ! -> 2bis
                    $userId = $url[3];
                    $this->returnToken($userId);
                } else { // ! -> 3
                    $tokenToVerify = $url[2];
                    $this->activationAccount($tokenToVerify);
                    header("Location: " . URL);
                    exit;
                }
                ...
                if (!empty($_POST['pseudo']) && !empty($_POST['emailInscription']) && !empty($_POST['password']) && !empty($_POST['confirmPassword'])) {
                    if (Securite::verifCSRF()) {
                        ...
                        //on crée le compte
                        $this->createAccount(trim($pseudo), trim($emailInscription), trim($password), trim($confirmPassword));
                    } else {
    ...
    private function activationAccount($tokenToVerify)
    {
        $jwt = new JWTService();
    ...
    private function returnToken($userId)
    {
